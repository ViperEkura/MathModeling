...
class AirQualityDataset(Dataset):
    def __init__(self, file_path, feature_columns, target_column, training):
        ...
        # 存储WSPM的索引位置
        self.wspm_idx = feature_columns.index('WSPM') if 'WSPM' in feature_columns else -1
...
def weighted_mse_loss(features, y_true, y_pred, wspm_idx, weight=2.0):
    base_loss = F.mse_loss(y_pred, y_true, reduction='none')
    
    if wspm_idx >= 0:
        # 获取WSPM特征值
        wspm_values = features[:, wspm_idx]
        # 创建权重张量
        weights = torch.ones_like(base_loss)
        weights[wspm_values < 1.0] = weight
        # 应用权重
...
def main():
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (features, targets) in enumerate(train_dataloader):
            ...
            loss = weighted_mse_loss(features, targets, outputs, 
                                   train_dataset.wspm_idx, wspm_weight)
        ...
        with torch.no_grad():
            for features, targets in test_dataloader:
                ...
                outputs = model(features)
                loss = weighted_mse_loss(features, targets, outputs,
                                       test_dataset.wspm_idx, wspm_weight)
